...
IFG.displayStackedBar = function(options) {
	...
	var svg = d3.select("#" + options.divId).html("").append("svg")
		.attr("width", function() {if (legendLocation == 'right') {
	...
	d3.json(options.jsonPath, function(error, data) {
	  ...
	  data.forEach(function(d) {
		var y0 = 0;
		d.categories = color.domain().map(function(name) {if(!d[name]) d[name] = 0; return {name: name, y0: y0, y1: y0 += +d[name], raw: d}; });
		d.total = d.categories[d.categories.length - 1].y1;
	  });

	  //data.sort(function(a, b) { return b.total - a.total; });

	  x.domain(data.map(function(d) { return d[options.categoryTitle]; }));
	  ...
	  var category = svg.selectAll("." + options.categoryTitle)
		...
		.enter().append("g")
		  .attr("class", "g")
		  .attr("transform", function(d) { return "translate(" + x(d[options.categoryTitle]) + ",0)"; });
	  ...
	  category.selectAll("rect")
		...
		.enter().append("rect")
		  ...
		  .on('mouseover', function(d, i) {d3tip.show(d, i);})
	  ...
	  legend.append("rect")
		  ...
		  .attr("width", 18)
		  .attr("height", 18)
		  .style("fill", function(d, i) {return colorsReverse[i]; });
