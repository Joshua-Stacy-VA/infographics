...
IFG.displayTreeMap = function(options) {
	...
	var display = function(jsonPath) {
		...
		svg = d3.select("#" + divId).html("").append("div")
		...
		d3.json(jsonPath, function(data) {
		  ...
		  cell.append("svg:rect")
			  ...
			  .style("fill", function(d) { return color(d.parent[nameTitle]); });
		  ...
		  cell.append("svg:text")
			  ...
			  .text(function(d) { return d[nameTitle].replace(" (", "-").replace(")", "") + " (" + d3.format(",")(d[valueTitle]) + ")"; })
			  .style("opacity", function(d) { d.w = this.getComputedTextLength(); return d.dx > d.w ? 1 : 0; });

		  d3.select("#" + divId).on("click", function() { zoom(root); });
