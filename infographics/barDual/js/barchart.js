...
IFG.displayBarChart = function(options) {
	...
	var margin = options.margin || {top: 40, right: 10, bottom: 30, left: 100},
	...
	var tip = d3.tip()
	  ...
	  .offset([-10, 0])
	  .html(function(d) {
		return "<strong>Name:</strong> <span style='color:white'>" + d[nameTitle] + "</span><br/><strong>" 
			+ valueTitleDisplay + ":</strong> <span style='color:white'>" + d3.format(",")(d[valueTitle]) + "</span><br/>"
			+ "<strong>" + valueTitleDisplay2 + ":</strong> <span style='color:white'>" + d3.format(",")(d[valueTitle2]) + "</span>";
	  })

	var svg = d3.select("#" + divId).html("").append("svg")
	...
	function setBars(data) {
		svg.selectAll(".bar")
		  ...
		  .enter().append("rect")
		  .attr("class", "bar")
		  .attr("x", function(d) {return x(d[XLabelTitle]);})	
	...
	function setBars2(data) {
		svg.selectAll(".bar2")
		  ...
		  .enter().append("rect")
		  .attr("class", "bar2")
		  .attr("x", function(d) {return x(d[XLabelTitle]) + x.rangeBand()/2;})	
		  .attr("width", x.rangeBand()/2)
		  .attr("y", function(d) { 
	...
	function setXYDomain(data) {
		x.domain(data.map(function(d) { return d[XLabelTitle]; }));
		//console.log(x.domain());
		y.domain([0, d3.max(data, function(d) { return d[valueTitle]; })]);
		y2.domain([0, d3.max(data, function(d) { return d[valueTitle2]; })]);
	}
