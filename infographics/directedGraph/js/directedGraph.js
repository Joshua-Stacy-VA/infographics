...
IFG.showDirectedGraph = function(options) {
	...
	function display(error, graph) {
	  var radius = d3.scale.linear()
	              .domain([0, d3.max(graph.nodes, function(n) { return n.value;})])
				  .range([4, 40]);
	  force
		  .nodes(graph.nodes)
		  .links(graph.links)
	...
	if(showArrow == 'showArrow') {
	   link.attr("marker-end", function(d) { return "url(#arrow)"; });
	}
	...
	var node = svg.selectAll(".node")
	  ...
	  link.append("title").text(function(d) { return d.source;});
